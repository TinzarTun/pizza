<!-- Navigation -->
<nav class="bg-white shadow-sm sticky top-0 z-50">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16">
            <div class="flex items-center gap-2">
                <div class="w-10 h-10 rounded-full pizza-bg flex items-center justify-center text-white font-bold">
                    <i class="fa-solid fa-pizza-slice"></i>
                </div>
                <span class="font-bold text-xl text-gray-900">Pizzaro</span>
            </div>
            <div class="nav-menu md:flex gap-8">
                <a href="menu.html" class="text-gray-700 hover:pizza-red">Menu</a>
                <a href="#offers" class="text-gray-700 hover:pizza-red">Offers</a>
                <a href="#services" class="text-gray-700 hover:pizza-red">Services</a>
                <a href="contact.html" class="text-gray-700 hover:pizza-red">Contact</a>
            </div>
            <div class="hidden md:flex gap-4 items-center">
                <a href="{{ route('page.login') }}" class="text-gray-700 hover:pizza-red font-medium">Login</a>
                <a href="{{ route('page.register') }}" class="px-6 py-2 rounded-lg text-white pizza-bg hover:opacity-90 font-medium">Sign Up</a>
            </div>
            <div class="hamburger" id="hamburger">
                <span></span>
                <span></span>
                <span></span>
            </div>
        </div>
    </div>
</nav>
